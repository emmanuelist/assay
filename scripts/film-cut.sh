...
subprocess.run(["ffmpeg","-y","-loglevel","error", *inputs,
                ...
                os.path.join(out,"assay-silent.mp4")], check=True)
...
PYEOF
  echo "Concatenating"
  ffmpeg -y -loglevel error -f concat -safe 0 -i "$OUT/list.txt" -c copy "$OUT/assay-silent.mp4"
fi

d=$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$OUT/assay-silent.mp4")
printf "\n  %s  (%.1fs)\n" "$OUT/assay-silent.mp4" "$d"
